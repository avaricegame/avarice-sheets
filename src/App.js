...
let characterSheet, weapons, armour, items, races, classes, customAbilities, spells, equipmentAbilities, money, characterSheetS
...
class App extends Component {
  ...
  addWeapon = (weaponName, rangedMelee, imageurl, range, proficiency, weaponType, requirements, abilityBonus, damage, uses, critical, magicalAbilities, description, published, createdBy) => {
    const newWeapon = {
      id: uniqid(),
      name: weaponName,
      rangedMelee,
  ...
  addArmour = (name, bodyArea, requirements, hp, speed, strength, dexterityReflex, constitutionFortitude, intelligence, charisma, perception, stealth, rangedAccuracy, meleeAccuracy, magicalAbilities, description, createdBy, published, imageurl) => {
    const newArmour = {
      id: uniqid(),
      name,
      bodyArea,
  ...
  addItem = (itemName, published, imageurl, itemDescription, createdBy) => {
    const newItem = {
      id: uniqid(),
      itemName,
      published,
  ...
  deleteCustomAbility = (id) => {
    console.log(id)
    this.setState({
      customAbilities: [...this.state.customAbilities.filter((customAbility) => customAbility.id !== id)],
    })
  ...
  render() {
    ...
    return (
      <div className="container">
        <Particles className="particles" params={particlesOptions} />
        <Header isSignedin={this.isSignedin} onRouteChange={this.onRouteChange} characterSheet={characterSheet} />
        <Navbar onRouteChange={this.onRouteChange} />

        {this.state.route === "gameplay" ? <Gameplay characterSheet={this.state.characterSheet} races={races} classes={classes} customAbilities={customAbilities} armour={armour} weapons={weapons} spells={spells} items={items} /> : this.state.route === "inventory" ? <Inventory characterSheet={this.state.characterSheet} money={this.state.money} editMoney={this.editMoney} weapons={this.state.weapons} armour={this.state.armour} items={this.state.items} equipmentAbilities={equipmentAbilities} addWeapon={this.addWeapon} addArmour={this.addArmour} addItem={this.addItem} deleteArmour={this.deleteArmour} deleteWeapon={this.deleteWeapon} deleteItem={this.deleteItem} /> : this.state.route === "stats" ? <Stats characterSheet={this.state.characterSheet} level={this.state.level} setLevel={this.setLevel} excellenceChips={this.state.excellenceChips} editExcellenceChips={this.editExcellenceChips} races={races} classes={classes} armour={armour} phyTotal={this.state.phyTotal} refTotal={this.state.refTotal} intTotal={this.state.intTotal} chaTotal={this.state.chaTotal} calculatePhyTotal={this.calculatePhyTotal} calculateRefTotal={this.calculateRefTotal} calculateIntTotal={this.calculateIntTotal} calculateChaTotal={this.calculateChaTotal} calculateReactionTotal={this.calculateReactionTotal} calculatePerceptionTotal={this.calculatePerceptionTotal} calculateSpeedTotal={this.calculateSpeedTotal} calculateFortitudeTotal={this.calculateFortitudeTotal} calculateStealthTotal={this.calculateStealthTotal} skillsStealth={this.state.skillsStealth} calculateSkillsStealth={this.calculateSkillsStealth} skillsExplosives={this.state.skillsExplosives} calculateSkillsExplosives={this.calculateSkillsExplosives} skillsUseMagicDevice={this.state.skillsUseMagicDevice} calculateSkillsMagic={this.calculateSkillsMagic} skillsMedicine={this.state.skillsMedicine} calculateSkillsMedicine={this.calculateSkillsMedicine} skillsTechnology={this.state.skillsTechnology} calculateSkillsTechnology={this.calculateSkillsTechnology} equipmentHp={equipmentHp} equipmentReflex={equipmentReflex} equipmentPhysique={equipmentPhysique} equipmentSpeed={equipmentSpeed} equipmentIntelligence={equipmentIntelligence} equipmentCharisma={equipmentCharisma} equipmentPerception={equipmentPerception} equipmentStealth={equipmentStealth} equipmentRangedAcc={equipmentRangedAcc} equipmentMeleeAcc={equipmentMeleeAcc} /> : this.state.route === "abilities" ? <Abilities characterSheet={this.state.characterSheet} races={races} classes={classes} customAbilities={this.state.customAbilities} spells={spells} deleteCustomAbility={this.deleteCustomAbility} addCustomAbility={this.addCustomAbility} /> : this.state.route === "info" ? <Info characterSheet={this.state.characterSheet} races={races} classes={classes} /> : this.state.route === "messages" ? <Messages /> : this.state.route === "notes" ? <Notes characterSheet={this.state.characterSheet} /> : <Error />}
        <Footer />
      </div>
    )
  }
