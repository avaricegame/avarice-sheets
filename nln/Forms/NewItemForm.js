...
class NewItemForm extends React.Component {
  ...
  onSubmit = (e) => {
    e.preventDefault()
    let { itemName, published, imageurl, itemDescription, createdBy } = this.state

    console.log(itemName, "CONSOLE LOGSAO  NAME")

    this.props.addItem(itemName, published, imageurl, itemDescription, createdBy)
    this.setState({
      currentItemName: "",
      itemName: "",
      published: "",
            ...
            {on && (
              <form onSubmit={this.onSubmit}>
                <fieldset>
                  ...
                  <select required name="currentItemName" value={currentItemName} onChange={this.onChange}>
                    <option></option>
